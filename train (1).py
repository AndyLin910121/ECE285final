...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input, hidden):
        embedded = input.view(1,1, self.hidden_size)
        #print(hidden.size())
        output = embedded
...
def trainIters(encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate=5):
    start = time.time()
    plot_losses = []
...
if train_flag=='True':
    ...
    trainIters(encoder1, attn_decoder1, 12000, print_every=1000)
    torch.save(encoder1.state_dict(),"encoder_model.pkl")
    torch.save(attn_decoder1.state_dict(),"decoder_model.pkl")
...
#     embedding_input_size=42446
    n_words=8877 ##changed here
    embedding_input_size=50940 ##changed here
    encoder1 = EncoderRNN(embedding_input_size, hidden_size).to(device)
    attn_decoder1 = AttnDecoderRNN(hidden_size, n_words, dropout_p=0.1).to(device)
    ...
    reference_df = reference_df[12000:]
...
#         output, hidden = self.gru(output, hidden)
#         print("output2 and hidden = " + str(output.shape) + str(hidden.shape))
#         return output, hidden
